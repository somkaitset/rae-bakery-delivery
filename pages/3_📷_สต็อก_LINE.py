...
with tab_list:
    ...
    if not ss:
        ...
        ss_sorted = sorted(
            ss,
            key=lambda r: (bills.parse_date(r.get("date")) or date(1900, 1, 1)),
            reverse=True,
        )
        ...
        for i in range(0, len(ss_sorted), cols_per_row):
            ...
            for j, row in enumerate(ss_sorted[i:i + cols_per_row]):
                with cols[j]:
                    img = storage.image_src(row.get("image"))
                    if img:
                        try:
                            st.image(img, use_container_width=True)
                        except Exception:
                            st.caption("(โหลดรูปไม่ได้)")
                    cust_name = customers_map.get(str(row.get("customer_code", "")), row.get("customer_code", ""))
                    prod_name = products_map.get(str(row.get("product_code", "")), row.get("product_code", ""))
                    st.markdown(
                        f"**{prod_name}** เหลือ **{row.get('remaining', '')}** ชิ้น  \n"
                        f"📅 {row.get('date', '')} • 🏪 {cust_name}"
                    )
                    if row.get("note"):
...
with tab_new:
    ...
    with st.form("new_stock_form", clear_on_submit=False):
        ...
        prod_options = {
            f"{p['name']} ({p['code']}) — กลุ่ม {p.get('price_group', '')}": p
        ...
        if submitted:
                ...
                try:
                    with st.spinner("กำลังบันทึกรูป..."):
                        image_url = storage.save_image(
                            name=f"stock_{selected_cust['code']}_{selected_prod['code']}_{stock_date.isoformat()}.jpg",
            ...
            try:
                sid = bills.create_stock(
                    ...
                    product_code=selected_prod["code"],
                    remaining=int(remaining),
                    image_url=image_url,
                    note=note.strip(),
                )
                st.success(
                    f"บันทึก `{sid}` — {selected_prod['name']} เหลือ {remaining} "
                    f"ที่ {selected_cust['name']}"
                )
            except Exception as e:
