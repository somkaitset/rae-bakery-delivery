...
def _load() -> tuple[pd.DataFrame, dict, list, list]:
    bs = sheets.bills()
    ...
    return df, customers_map, products_list, its


try:
    df_all, customers_map, products_list, its = _load()
...
# --- Tabs ---
tab_d, tab_w, tab_m, tab_c, tab_p = st.tabs(
    ["📅 รายวัน", "🗓️ รายสัปดาห์", "📆 รายเดือน", "👥 รายลูกค้า", "🍰 รายสินค้า"]
)
...
with tab_p:
    # bill_ids in the date-filtered, all-statuses frame → inherits both filters
    allowed_ids = set(df["รหัสใบส่ง"].astype(str))
    rows = bills.summarize_products_by_code(its, allowed_ids, products_list)
    prod_df = pd.DataFrame(rows)

    if prod_df.empty:
        st.info("ยังไม่มีสินค้า")
    else:
        # chart from English-keyed frame BEFORE rename (top 20 by amount)
        st.bar_chart(
            prod_df.head(20).set_index("name")["amount_total"],
            height=300,
            use_container_width=True,
        )
        display_df = prod_df.rename(columns={
            ...
            "name": "ชื่อสินค้า",
            "qty_total": "จำนวนชิ้นรวม",
            "amount_total": "ยอดเงินรวม",
        })[["รหัสสินค้า", "ชื่อสินค้า", "จำนวนชิ้นรวม", "ยอดเงินรวม"]]
        st.dataframe(
            display_df,
            use_container_width=True,
            hide_index=True,
            column_config={
                "ยอดเงินรวม": st.column_config.NumberColumn(format="%.2f"),
                "จำนวนชิ้นรวม": st.column_config.NumberColumn(format="%d"),
            },
        )
